...
void calc_tentative_dist(vertex_t *key, binheap_node_t *h){
  ...
  printf("X: %d, Y: %d\n", with->position[dim_x], with->position[dim_y]);
  /*
...
void get_neighbors(vertex_t *v,   
  ...
  if(v->position[dim_x] > 0 && v->position[dim_x] < 78){
    ...
    if(v->position[dim_y] < 19){
      memcpy(n[bot_right], arr[v->position[dim_y] + 1][v->position[dim_x] + 1], sizeof(*arr[v->position[dim_y] + 1][v->position[dim_x] + 1]));
      //calc_tentative_dist(v, n[bot_right]);
    }
    //Top Right
    if(v->position[dim_y] > 1){
     memcpy(n[top_right], arr[v->position[dim_y] - 1][v->position[dim_x] + 1], sizeof(*arr[v->position[dim_y] - 1][v->position[dim_x] + 1]));
      //calc_tentative_dist(v, n[top_right]);
    }
  }
  //Left
  if(v->position[dim_x] > 1 && v->position[dim_x < 79]){
    memcpy(n[left], arr[v->position[dim_y]][v->position[dim_x]-1], sizeof(*arr[v->position[dim_y]][v->position[dim_x]-1]));
    //calc_tentative_dist(v, n[left]);
    //Bot Left
    if(v->position[dim_y] < 19){
      memcpy(n[bot_left], arr[v->position[dim_y] + 1][v->position[dim_x] - 1], sizeof(*arr[v->position[dim_y] + 1][v->position[dim_x] - 1]));
  ...
  //Top
  if(v->position[dim_y] > 1){
    memcpy(n[top], arr[v->position[dim_y] - 1][v->position[dim_x]], sizeof(*arr[v->position[dim_y] - 1][v->position[dim_x]]));
    //calc_tentative_dist(v, n[top]);
  }
  //Bottom
  if(v->position[dim_y] < 19){
    memcpy(n[bot], arr[v->position[dim_y] + 1][v->position[dim_x]], sizeof(* arr[v->position[dim_y] + 1][v->position[dim_x]]));
